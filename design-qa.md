...
- 셀러 보내기 원본에서 Figma 상단 크롬을 실제로 제거한 content-only 자산을 추가했다.
- 주류 선택과 미리보기 원본에 구워진 액션바도 제거하고, 실제 고정 하단바를 한 번만 표시했다.
- 이미지를 위로 끌어올리던 좌표 보정을 제거해 시각적 버튼과 터치 영역이 같은 원점을 쓰게 했다.
- 라이브앱 진입 전 기능 카드의 이미지 영역을 16:9로 통일했다.

## Verification

- 친구 선택 → 주류 선택 → 보낼 셀러 미리보기를 모바일 프레임에서 직접 실행했다.
- 각 화면에서 탑앱바는 하나만 나오고, 주류 선택과 미리보기의 고정 하단바도 하나만 나오는 것을 확인했다.
- 친구와 주류 선택 시 이미지 상태와 버튼 활성 상태가 정상적으로 바뀌는 것을 확인했다.
- 전체 흐름 페이지의 카드 이미지가 16:9로 렌더링되는 것을 확인했다.
